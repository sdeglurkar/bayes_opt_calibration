...
class PickToLearn():
    ...
    def get_error_gp_dataset(self):
        if os.path.isfile(f"error_gp_data_{NUM_ERROR_GP_POINTS}.pkl"):
            ...
            error_gp_candidates, error_gp_true_costs = get_ground_truths_for_random_points(range_x, RNG, F, NUM_ERROR_GP_POINTS)
    ...
    def get_acquisition_fn_calib_dataset(self):
        if os.path.isfile(f"acq_calibration_data_{NUM_CALIBRATION_POINTS}.pkl"):
            ...
            acq_calib_candidates, acq_calib_true_costs = get_ground_truths_for_random_points(range_x, RNG, F, NUM_CALIBRATION_POINTS)
...
bo_init_iters = NUM_MODEL_INIT_ITERS
if MULTIPLE_SEEDS:
    bols_list = []
    ...
    for i in range(len(bols_list)):
        ...
        original_cand_len = plot_main_gp(bols, grid, candidates, oned_x, all_values, theta_index, 
...
if MULTIPLE_SEEDS:
    ...
    for i in range(len(bols_list)):
        ...
        _ = plot_main_gp(bols, grid, candidates, oned_x, all_values, theta_index, 
                            BETA, fig_name, fig_name_colorbar, mile_name)
...
if MULTIPLE_SEEDS:
    ...
    for i in range(len(bols_list)):
        bols = bols_list[i]
        seed = MULTIPLE_SEED_LIST[i]
        tpr, fpr, tnr, fnr = validate_final_level_set(bols, validation_candidates, 
                                                            validation_true_costs, BETA)
